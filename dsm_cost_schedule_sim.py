...
class SimulatorGUI:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(cost_bottom, text="入力状態を保存", command=self.save_input_state).pack(side="left", padx=6)
    ...
    @staticmethod
    def _bind_enter_navigation_grid(
        rows: List[List[ttk.Entry]],
        on_enter: Callable[[ttk.Entry], None] | None = None,
    ) -> None:
        if not rows:
            return
        ...
        for idx, entry in enumerate(flattened):
            ...
            def on_enter_key(_event, cur=entry, nxt=next_entry):
    ...
    def _rebuild_ic_table(self, n: int) -> None:
        ...
        def sync_row_for_entry(cur_entry: ttk.Entry) -> None:
            for row_idx, row_cells in enumerate(ic_nav_rows):
                if cur_entry in row_cells:
                    self._sync_ic_row_to_cost_schedule(row_idx)
                    break

        self._bind_enter_navigation_grid(ic_nav_rows, on_enter=sync_row_for_entry)
