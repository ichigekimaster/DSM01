...
class SimulatorGUI:
    ...
    def _rebuild_tables(self) -> None:
        ...
        self.use_reordered_view = False
        self.reorder_status_var.set('現在順序: 1,2,3,...')
    ...
    def _compute_lower_triangular_order(self, matrix: List[List[float]]) -> List[int]:
        ...
        return [i for i, _net, _in in stats]
    ...
    def _reorder_dsm_visualization(self) -> None:
        ...
        new_order = self._compute_lower_triangular_order(matrix)
        self._write_reordered_values(new_order, task_rows, matrix)

        self.use_reordered_view = False
        self.visual_order = list(range(n))
        self.reorder_status_var.set(
            f"現在順序: {self._format_order_text(old_order)}  →  新順序: {self._format_order_text(new_order)}"
        )
        self._update_column_headers()
        self._refresh_dsm_visualization()
